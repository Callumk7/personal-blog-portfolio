...
export function getRecentPostFrontmatter(count: number): PostFrontmatter[] {
...
// a function that returns posts of a certain category
export function getPostsByCategory(category: string): PostFrontmatter[] {
  const postFrontmatter = getAllPostFrontmatter();
  const filteredPosts = postFrontmatter.filter((post) => post.category === category);
